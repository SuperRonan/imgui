...
class Test
{
    ...
    {
        child.open = true;
        child.dock_id = GetOrCreateWindowDockID(window);
        if (split_open)
        {
            ImGuiID parent = child.dock_id;
            ImGuiDockNode* node = ImGui::DockBuilderGetNode(parent);
            node->SplitAxis;
